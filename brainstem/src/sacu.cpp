	...
	/* Ensure we can reuse socket without TIME_WAIT problems */
	setsockopt(serversock, SOL_SOCKET, SO_REUSEADDR, &reuse_addr, sizeof(reuse_addr));
	...
	memset((char *) &connectlist, 0, sizeof(connectlist));	/* Zero off the connect list before we begin */
	    ...
	    //readsocks=select(maxfd, &readfs, NULL, NULL, NULL);
	    ...
	    if (FD_ISSET(usonicfd, &readfs)) /* Input from ultrasonics available */
			...
			if (sensor>5) { printf("\n"); sensor=1; }
	  		outbuf[0] = sensor;
          		...
          		usonicres=write(usonicfd, outbuf, 2);
			gettimeofday(&usonictv, NULL);
		    }
	    }
